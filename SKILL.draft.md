...
2. 找到它在现有文档结构中最合适的位置。
3. 根据需要新增、拆分、移动或重组小节。
4. 检查更新后的文档是否还存在阅读路径断裂；如果有，按“可读性修复”处理（见后文）。
5. 回复保持简短，文档中已经传达过的信息不要再输出一遍，用户自己会去看文档的。比如你已经在文档中写了某一个知识，你就不要再用另一种说法去讲这个知识，直接跟用户说你添加了这部分内容就行。
...
在写教学文档前，先根据已有对话和文档内容建立一个读者模型。不要只用“初学者 / 中级 / 高级”这种粗标签，而要写成前置知识判断：
